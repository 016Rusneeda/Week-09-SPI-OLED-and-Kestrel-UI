# บทเรียนที่ 9.2: ชุดคำสั่งควบคุม SSD1306, วงจรทวีแรงดัน (Charge Pump) และ Addressing Modes

---

## 1. วงจรทวีแรงดันภายใน (Internal DC-DC Charge Pump) ทำไมหน้าจอถึงไม่ติด?
...
⚠️ **ประเด็นสำคัญ:** ค่าเริ่มต้นจากโรงงาน (Reset Default) ของวงจร Charge Pump นี้คือ **Disabled (ปิดอยู่)**  
ดังนั้น หากเฟิร์มแวร์ไม่ได้ส่งคำสั่งเปิด Charge Pump ต่อให้สั่ง `Display ON (0xAF)` หน้าจอก็จะมืดสนิทเพราะไม่มีแรงดันไฟฟ้าไปจุดเม็ด OLED!

---

## 2. ลำดับคำสั่งมาตรฐานในการเริ่มต้นระบบ (Magic Initialization Sequence)
...
## 3. โหมดการเข้าถึงหน่วยความจำ (Memory Addressing Modes)

SSD1306 มีโหมดการเลื่อนตำแหน่งหน่วยความจำ (Pointer Addressing) 3 รูปแบบ:
...
> 💡 **ทำไมเราจึงเลือกใช้ Horizontal Addressing Mode (`0x20, 0x00`):**  
...
## 4. กลไกการประหยัดพลังงานและการปรับแต่ง (Display Dimming & Inversion)
...
2. **การกลับสีหน้าจอ (Invert Display):**
